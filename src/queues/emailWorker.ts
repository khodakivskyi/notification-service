...
class EmailWorker {
    ...
    async executeJob(job: EmailJob): Promise<boolean> {
        ...
        if (job.type === 'verification') {
            ...
            await emailService.sendNotification(job.data.to || job.data.email!, job.data.subject!, job.data.message!);
        } else {
    ...
    shouldRetry(error: any, currentRetries: number, maxRetries: number): {
        willRetry: boolean;
        isNonRetriable: boolean
    } {
        const isNonRetriable =
            error instanceof ValidationError ||
            (error?.isOperational && typeof error.statusCode === 'number' && error.statusCode >= 400 && error.statusCode < 500);
